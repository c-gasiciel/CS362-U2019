...
* * DESCRIPTION:    Random test for Minion card functionality
******************************************************************************/
#include "dominion.h"
...
void checkMinion(int choice1, int choice2, struct gameState *post, int currPlayer, int handPos)
{
  //Iterators
  int i;
  ...
  playMinion(choice1, choice2, post, currPlayer, handPos);
...
int main()
{
  ...
  while(testCounter < tests)
  {
    ...
    //Call minion test
    checkMinion(choice1, choice2, &TestGame, currPlayer, handPos);
